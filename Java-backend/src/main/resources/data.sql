...
UPDATE menu SET parent_id = NULL WHERE code IN ('admin-dashboard', 'kb-management', 'user-management', 'token-analysis', 'resource-management');
...
SELECT 'student', id FROM `menu` WHERE code IN ('dashboard', 'plan-list', 'knowledge-tree', 'note-list', 'analytics', 'profile', 'knowledge-graph');

INSERT IGNORE INTO role_menu (role, menu_id)
SELECT 'admin', id FROM `menu`
WHERE code IN ('admin-dashboard', 'kb-management', 'user-management', 'token-analysis', 'resource-management');

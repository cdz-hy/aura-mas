...
import { getLogPage, getLogStats } from '@/api/admin'
import type { AdminLogItem, AdminLogStats, AdminLogQuery } from '@/api/admin'
  ...
  return [
    {
      ...
      bgClass: 'bg-blue-50', iconClass: 'text-blue-500',
    },
    {
      label: '今日新增',
      value: s?.todayCount ?? '--',
      icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>',
      bgClass: 'bg-emerald-50', iconClass: 'text-emerald-500',
    },
    {
      label: '失败操作',
      value: s?.failCount ?? '--',
      ...
      bgClass: 'bg-red-50', iconClass: 'text-red-500',
    },
    {
      label: '涉及模块',
      value: s?.moduleDistribution?.length ?? '--',
      ...
      bgClass: 'bg-purple-50', iconClass: 'text-purple-500',
    },
  ]
})
...
function applyPreset(preset: { label: string; days: number }) {
  ...
  if (preset.days === -1) {
    ...
    dateRange.value.start = dayjs().format('YYYY-MM-DD')
...
async function loadStats() {
  try {
    const res = await getLogStats() as any

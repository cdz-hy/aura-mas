...
async def generate_learning_report(request: LearningReportRequest):
    """生成AI学习报告（SSE流式返回，当天缓存到 Redis，次日自动失效）"""
    from datetime import datetime, timedelta

    user_id = request.user_id or 0
    today = datetime.now().strftime("%Y-%m-%d")
    cache_key = f"aura:report:{user_id}:{today}"
    ...
    async def generate():
        try:
            # 1. 尝试从 Redis 获取今天的缓存报告
            redis = _get_redis()
            if redis:
                try:
                    cached = redis.get(cache_key)
                    if cached:
                        logger.info(f"[学习报告] 命中缓存: user={user_id}, date={today}")
                        yield f"data: {json.dumps({'type': 'chunk', 'content': cached}, ensure_ascii=False)}\n\n"
                        yield f"data: {json.dumps({'type': 'done'}, ensure_ascii=False)}\n\n"
                        return
                except Exception as e:
                    logger.warning(f"[学习报告] 读取缓存失败: {e}")

            # 2. 缓存未命中，调用 LLM 生成
            logger.info(f"[学习报告] 缓存未命中，调用 LLM 生成: user={user_id}, date={today}")
            ...
            record_from_mimo(llm, user_id, "analytics_report")

            # 3. 写入 Redis 缓存（TTL 到当天 23:59:59）
            if redis and full_report:
                try:
                    redis.setex(cache_key, ttl_seconds, full_report)
                    logger.info(f"[学习报告] 已缓存: key={cache_key}, ttl={ttl_seconds}s")
                except Exception as e:
                    logger.warning(f"[学习报告] 写入缓存失败: {e}")

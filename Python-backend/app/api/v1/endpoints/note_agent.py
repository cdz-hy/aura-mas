"""
笔记智能整理 API - 将杂乱笔记内容整理为结构化 Markdown
SSE 流式输出整理结果
"""
import json
import asyncio
import logging
from fastapi import APIRouter
from fastapi.responses import StreamingResponse
from pydantic import BaseModel, ConfigDict, Field
from app.services.db.java_client import java_client
from app.agents.llm_factory import get_note_format_llm
from app.utils.profile_utils import ensure_learning_behavior_fields
from app.skills.loader import load_skill
from app.utils.token_recorder import record_from_mimo

logger = logging.getLogger("api.note_agent")
router = APIRouter()


class FormatRequest(BaseModel):
    """Note format request. Accepts camelCase aliases from the Vue client."""

    model_config = ConfigDict(populate_by_name=True)

    ticket: str
    content: str
    instruction: str = ""  # 用户自定义整理要求
    # Optional capture context from the learning workbench
    excerpt: str = ""  # 摘录原文（来源材料，不可被整理结果覆盖）
    source_title: str = Field(default="", alias="sourceTitle")  # 来源标题
    note_type: str = Field(default="", alias="noteType")  # excerpt | quick | question

FORMAT_PROMPT_BASE = """你是一位专业的学习笔记整理师，擅长把杂乱内容变成结构清晰、排版精美的笔记。请将以下笔记内容整理为结构化 Markdown。

## 排版规范

### 1. 重点标记
用简洁的文字标记代替符号装饰：
- 核心概念 / 关键定义：用 **加粗** 标注
- 重点内容 / 必背知识：用 `> **重点**：` callout 提示
- 小技巧 / 实用提示：用 `> **技巧**：` callout 提示
- 易错点 / 注意事项：用 `> **注意**：` callout 提示
- 举例说明：用 `> **示例**：` 引出
- 步骤流程：用 **Step 1**、**Step 2** 格式
- 对比分析：优先使用表格
- 总结结论：用 `**总结**：` 开头

### 2. Callout 信息卡片
重要提示用 blockquote + 加粗标签，形成醒目的信息卡片：
```
> **技巧**：这里的提示内容
> **注意**：这里写易错点
> **重点**：这里写核心概念
> **示例**：这里写具体例子
> **关键**：这里写核心原理
```

### 3. 排版节奏
- **短句分段**：每 2-3 句一个段落，段间留空行，增加呼吸感
- **关键词加粗**：重要术语用 `**加粗**` 高亮
- **行内代码**：专业术语、函数名、变量名用 `` ` ` ` `` 包裹
- **不要大段文字**：宁可多分段，也不要一大坨文字堆在一起

### 4. 结构化呈现
- 标题层级：`#` 一级 → `##` 二级 → `###` 三级
- 列表要点用 `- ` 无序列表
- 流程步骤用 **Step N:** 格式
- 有对比关系的内容用表格呈现
- 代码用 ``` 包裹并标注语言
- 数学公式用 $...$ 或 $$...$$

### 5. 开头概览
如果内容较多，在开头用 `> **一句话概括**：xxx` 给出整体概览

### 6. 末尾总结
在末尾添加 `## 一句话总结` 用 3-5 个要点精炼概括

## 核心原则

1. **保留原文**：学习材料原文不可改写；用户补充有实质内容时保留其语义
2. **去除冗余**：删除重复内容、无意义语气词、乱码、占位词
3. **逻辑分组**：相关内容归到同一标题下，有条有理
4. **视觉舒适**：排版要有节奏感，不要密密麻麻，也不要过于稀疏
5. **避免使用 emoji 表情**：不要在笔记中使用任何 emoji 符号（如🔑📌💡⚠️🌰📖等），用纯文字和 Markdown 格式实现视觉层次
6. **必须有可见整理效果**：整理后的 note 内容应比输入更清晰（标题/列表/加粗/callout/批注），禁止原样照抄

## 分段补充笔记（硬性要求，优先级最高）

客户端在「原文 + 补充」穿插笔记上，会下发「只读原文上下文 + 可写 note 块」，例如：

### 第 1 段
【对应原文·只读·禁止写入输出·禁止改写】
……学习材料原文……

【用户补充·请整理进下方 note 块】
<<<note index="0">>>
用户补充（可能为空/很短）
<<< /note >>>

**必须遵守：**
1. **只输出**各 `<<<note index="N">>>` … `<<< /note >>>` 块（可含内部 Markdown），禁止输出 `<<<excerpt>>>`，禁止把「对应原文」原文再抄进输出
2. 每一个 note 标记与 index 必须保留，禁止删除、合并、拆分、改编号
3. **禁止**把多个 note 合并成一篇连续文章
4. note 内写「学习补充」：提炼要点、结构层次、易错提醒、一句话理解等
5. **若用户补充为空、过短（如几个字）、或无意义**，必须根据该段【对应原文】生成结构化学习补充（仍只写在 note 内），例如：
   - `> **一句话概括**：…`
   - 3-6 条要点列表（关键术语 **加粗**）
   - 至少 1 处 `<<A:编号|类型|文本>>` 批注
6. 若用户补充有实质内容：在保留其意思的前提下排版美化，并补全结构与批注
7. 批注标记必须是完整形式 `<<A:编号|类型|文本>>`，写在 note 块内部；**禁止**输出空的 `<<>>`、`<<<>>>`，或单独成行的 `<<` / `>>` / `>>>`

若输入中出现完整的 `<<<excerpt>>>` 原文块（兼容旧请求）：
- 每一个 excerpt 块必须原样保留，禁止改写/删除/移动
- 只整理 excerpt 之外的用户补充文字
- 输出保持「原文块 → 用户笔记 → …」穿插顺序

## 学习风格适配

{style_hints}

## 边栏批注（必须执行）

整理笔记时，你必须在正文中标注学生需要注意的地方。标注方法：在对应句子的句号后面插入一个批注标记。

批注标记格式为：<<A:编号|类型|批注文本>>

其中：
- 编号：从1开始递增的数字
- 类型：只能是这5种之一：易混淆、易错点、提醒、注意、技巧
- 批注文本：15-40字，具体说明需要注意什么
- 禁止输出残缺标记（如 `<<>>`、只有 `>>` 的行）；必须成对完整

### 示例（你的输出必须包含这样的标记）

UDP 是无连接的运输层协议，发送数据前不需要建立连接。<<A:1|易混淆|注意和TCP的区别：TCP需要三次握手建立连接，UDP不需要>>

UDP 首部只有8字节，包含源端口、目的端口、长度、校验和四个字段。<<A:2|技巧|记忆口诀：源目长校，8字节四字段>>

UDP 的校验和在IPv6中是强制的。<<A:3|易错点|IPv4中校验和可选，但IPv6中强制，考试常考这个区别>>

### 硬性要求
- 每篇笔记至少标注2处，不能跳过
- <<A:编号|类型|文本>> 放在句号之后
- 标记是内容的一部分，必须输出

## 输出

直接输出整理后的 Markdown。输出中必须包含 <<A:编号|类型|文本>> 批注标记（见上方示例），这是内容的一部分。不要输出任何解释说明。"""


def _build_style_hints(learning_behavior: dict) -> str:
    """根据用户学习风格生成整理提示"""
    hints = []
    vv = learning_behavior.get("visual_vs_verbal", 0)
    si = learning_behavior.get("sensing_vs_intuitive", 0)
    sg = learning_behavior.get("sequential_vs_global", 0)

    if vv < -0.2:
        hints.append("- 用户偏好视觉型学习：多用表格对比、流程图描述，在关键概念处标注 `[建议配图]`")
    elif vv > 0.2:
        hints.append("- 用户偏好言语型学习：展开详细文字解释，用完整的句子和比喻描述概念")

    if si < -0.2:
        hints.append("- 用户偏好感知型学习：多举具体实例和实际应用场景，贴近生活")
    elif si > 0.2:
        hints.append("- 用户偏好直觉型学习：先给出概念框架和核心原理，再补充细节")

    if sg < -0.2:
        hints.append("- 用户偏好循序型学习：用 Step N 严格按步骤排列，确保前后衔接")
    elif sg > 0.2:
        hints.append("- 用户偏好全局型学习：开头给出概览大纲，先见森林再见树木")

    return "\n".join(hints) if hints else "无特殊偏好，按通用方式整理。"


@router.post("/note/format")
async def format_note(req: FormatRequest):
    """笔记智能整理 - SSE 流式返回整理后的 Markdown"""

    async def event_stream():
        try:
            # 认证（同步调用，放到线程池避免阻塞事件循环）
            user_info = await asyncio.to_thread(java_client.validate_ticket, req.ticket)
            user_id = user_info.get("userId") or user_info.get("user_id")
            if not user_id:
                yield f"data: {json.dumps({'type': 'error', 'content': '认证失败'}, ensure_ascii=False)}\n\n"
                return

            # 获取用户画像（同步调用，放到线程池）
            try:
                profile = await asyncio.to_thread(java_client.get_user_profile, user_id)
                behavior = profile.get("learning_behavior", {})
                behavior = ensure_learning_behavior_fields(behavior)
            except Exception:
                behavior = {}

            # 构造 prompt
            style_hints = _build_style_hints(behavior)
            system_prompt = FORMAT_PROMPT_BASE.format(
                style_hints=style_hints,
            )

            content = req.content or ""
            has_note_slots = "<<<note" in content
            has_excerpt_fences = "<<<excerpt" in content
            user_prompt = (
                "请整理以下笔记。注意：输出中必须包含 <<A:编号|类型|文本>> 批注标记，至少2处。\n\n"
            )
            if has_note_slots:
                user_prompt += (
                    "本请求按段提供【对应原文·只读】与【用户补充 note 块】。"
                    "你的输出只能包含整理后的 <<<note index=\"N\">>> 块；"
                    "必须保留全部 note 标记与 index；不要合并成一篇文章；"
                    "不要把原文抄进输出；不要输出 <<<excerpt>>>。"
                    "若 note 内为空/过短/无意义，必须根据该段原文写出结构化学习补充（要点+批注），禁止原样照抄。\n\n"
                )
            elif has_excerpt_fences:
                user_prompt += (
                    "本笔记为「原文块 + 用户补充」穿插结构。"
                    "所有 <<<excerpt>>> 块必须原样保留；只整理 excerpt 之外的用户补充文字；"
                    "不要把原文与补充合并成一篇纯文章。\n\n"
                )
            if req.instruction.strip():
                user_prompt += f"用户特别要求：{req.instruction.strip()}\n\n"

            # Capture workbench context: excerpt is source material; content is learner thinking
            context_parts = []
            if req.source_title and req.source_title.strip():
                context_parts.append(f"学习来源：{req.source_title.strip()}")
            if req.note_type and req.note_type.strip():
                context_parts.append(f"笔记类型：{req.note_type.strip()}")
            if req.excerpt and req.excerpt.strip():
                context_parts.append(
                    "摘录原文摘要（仅供理解上下文，禁止写入整理结果、禁止改写）：\n"
                    f"{req.excerpt.strip()}"
                )
            if context_parts:
                user_prompt += "\n".join(context_parts) + "\n\n"

            if has_note_slots:
                user_prompt += "待整理的补充笔记分段（请按 note 块分别整理并原样保留标记）：\n" + content
            else:
                user_prompt += "完整笔记正文（请按规则整理）：\n" + content
            messages = [
                {"role": "system", "content": system_prompt},
                {"role": "user", "content": user_prompt},
            ]

            yield f"data: {json.dumps({'type': 'progress', 'content': '正在整理笔记...'}, ensure_ascii=False)}\n\n"

            # 流式调用 LLM
            llm = get_note_format_llm()
            formatted = ""
            for chunk in llm.chat_stream(messages):
                if chunk:
                    formatted += chunk
                    yield f"data: {json.dumps({'type': 'chunk', 'content': chunk}, ensure_ascii=False)}\n\n"
            record_from_mimo(llm, user_id, "note_formatting")

            yield f"data: {json.dumps({'type': 'done', 'content': formatted}, ensure_ascii=False)}\n\n"

        except Exception as e:
            logger.error(f"笔记整理失败: {e}")
            yield f"data: {json.dumps({'type': 'error', 'content': str(e)}, ensure_ascii=False)}\n\n"

    return StreamingResponse(
        event_stream(),
        media_type="text/event-stream",
        headers={
            "Cache-Control": "no-cache",
            "Connection": "keep-alive",
            "X-Accel-Buffering": "no",
        },
    )

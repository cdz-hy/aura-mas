...
9. 如果提供了「可用的相关图片」，在回答中引用最相关的图片（最多2张），使用 Markdown 图片语法 ![描述](URL)。只引用与回答内容密切相关的图片，如果图片与问题无关则不引用。图片引用应自然融入回答内容中"""
...
请分析用户问题与模块内容的相关性，输出 JSON:"""
    ...
    # 6. RAG 不足时，网络搜索（简单问题、页面问题、意图不明确时跳过）
    web_context = ""
    logger.info(f"  [辅导智能体] 网络搜索判断: is_simple={is_simple}, is_page_question={is_page_question}, rag_context为空={not rag_context}")
    if not is_simple and not is_page_question and not rag_context and not no_resource_clarification:

  ...
  "issues": [
    {
      ...
      "description": "问题描述（单行文本）",
      "source_index": 0,
      "suggestion": "修改建议（单行文本）"
    }
  ],
  "suggestions": ["优化建议1", "优化建议2"],
  "summary": "审查总结（单行文本，不要包含换行符）"
}
    ...
    try:
        ...
        for i, result in enumerate(review_results):
            if isinstance(result, Exception):
                ...
                passed_modules.append(i + 1)  # 异常默认通过
                continue
        ...
        logger.error(f"  [审查智能体] 模块审查异常: {str(e)}")
        logger.info(f"{'='*60}")
        return {
            "review_passed": True,  # 异常默认通过
            "review_feedback": f"模块审查异常: {str(e)}",
            "review_suggestions": [],
            "failed_modules": [],
            "passed_modules": list(range(1, len(module_list) + 1)),

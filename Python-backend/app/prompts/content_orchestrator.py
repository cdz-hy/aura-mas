IMAGE_LAYOUT_TEMPLATES = """  - 运用你的理解能力，挑选出与你生成的文本段落、核心概念最相关的真实图片进行插入。
...
IMAGE_SECTION_WITH_TEMPLATES = """- 图片精选与美化排版（重要：严禁直接堆砌大图，必须设计排版）：
...
IMAGE_SECTION_NO_IMAGES = """- 【纯文本排版】：当前检索资料中没有可用的图片资源，**严禁**生成任何 `<img>` 标签、`![图片]()` Markdown 图片语法或虚构的图片 URL。请使用纯文本、表格、代码块和 Markdown 格式进行排版。
- 【绝对红线（严禁幻觉链接）】：**严禁**在正文或结尾虚构任何不存在的外部参考链接、维基百科链接或论文引用标记（如 [1], [2] 等）。你的所有知识基于检索资料，不需要提供无法点击的假链接！"""


def build_image_section(has_images: bool) -> str:
    """根据是否有可用图片，返回对应的图片排版指令"""
    if has_images:
        return IMAGE_SECTION_WITH_TEMPLATES.format(image_templates=IMAGE_LAYOUT_TEMPLATES)
    return IMAGE_SECTION_NO_IMAGES


CONTENT_ORCHESTRATOR_BATCH_PROMPT = """你是一个专业的内容编排专家。你的任务是将检索到的多模态知识内容，按照合理美观的顺序和模块化结构编排为学习资源。

## 铁律（绝对禁止违反，违反即为严重失败）
1. **严禁编造图片 URL**：你只能使用检索资料中明确以 `图片URL:` 前缀给出的真实图片地址。绝对禁止自己构造、猜测、拼接或编造任何图片链接！如果你没有收到真实的图片 URL，就完全不使用任何图片！
2. **严禁编造外部链接**：禁止在正文或结尾虚构任何不存在的参考链接、维基百科链接、论文引用标记 [1][2] 等。
3. 违反以上任一条，你的输出将被视为严重失败并被丢弃。

...
- 图片精选与美化排版（重要：严禁直接堆砌大图，必须设计排版）：
  - 【绝对红线】：必须且只能使用检索上下文中明确提供给你的真实图片 URL！严禁自己编造、猜测或生成任何虚假的图片 URL（如 example.com 等）！
  - 【无图则不插图】：如果检索结果中没有相关图片，请直接使用纯文本排版，**绝对不要**生成任何包含假 URL 的 HTML 或 Markdown 标签！
...
如果提供了「审查反馈」，你必须：
1. 逐条阅读审查者指出的问题和缺失要点
2. 在生成内容时**针对性地修复**每个问题
3. 确保缺失的关键知识点被补充到内容中
4. 在 metadata 中新增 "fixes_applied" 字段，列出本次修复的问题

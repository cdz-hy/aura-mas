...
- **preferred_quiz_preference**: 偏好题目配置，包含三种属性：
  - `types` (字符串列表): 偏好的题目类型列表，必须为中文名称，如 "选择题", "判断题", "填空题", "简答题", "多选题"
  - `count` (整数或 null): 偏好的生成题目数量
  - `difficulty` (字符串或 null): 偏好的题目难度（如 "高难度", "中等难度", "简单" 等）
...
严格输出 JSON：
{
  ...
  "updates": {
    ...
    "preferred_quiz_preference": {
      "types": ["选择题"]等中文列表或 null,
      "count": 数量整数或 null,
      "difficulty": 难度字符串或 null
    } 或 null,
...
2. **列表字段**: 输出的是本次需要**新增**的项，系统会自动合并到现有列表中。
   - 设为 `null` 表示不更新此字段
   - 设为 `[]` 表示不新增任何项
3. **列表字段删除**: 使用 `{字段名}_remove` 格式输出需要删除的项。
   - 例如：`"interest_tags_remove": ["Python"]` 表示从兴趣标签中删除 "Python"
   - 支持删除的字段：`knowledge_base_remove`、`weak_areas_remove`、`interest_tags_remove`、`preferred_resource_types_remove`
4. **分类字段**: `goal_orientation` 直接输出目标值（如 "exam"），不是增量
5. **置信度**: 影响所有增量字段的调整幅度，低置信度时调整更保守
6. **只更新有明确证据的字段**: 不确定的设为 null
7. **严禁使用 emoji 表情符号**
8. **偏好题目配置维护（重要）**：如果接收到「用户本次出题的专门要求」（即检测到的题型、数量、难度），并且这些要求是用户主动且明确提出的，请基于这些出题要求和原有的配置来完善 `preferred_quiz_preference` 字段（例如：用户刚才指定了要生成十个最难的选择题，则在 updates 的该字段中写入题型“选择题”，数量为 10，难度为“高难度”）。
...
### 示例4：兴趣标签删除
当前画像: interest_tags = ["Python", "Java", "前端"]
用户说："我现在对 Python 不太感兴趣了，可以去掉"

输出：
{
  "should_update": true,
  "update_reason": "用户移除不感兴趣的标签：Python",
  "updates": {
    "interest_tags_remove": ["Python"]
  },
  "confidence": 0.95,
  "analysis": "用户明确表示对 Python 不再感兴趣，建议删除该标签"
}

### 示例5：同时新增和删除
当前画像: interest_tags = ["Python", "数据分析"]
用户说："我对 Python 不感兴趣了，但是对机器学习很感兴趣"

输出：
{
  "should_update": true,
  "update_reason": "移除 Python 标签，新增机器学习标签",
  "updates": {
    "interest_tags": ["机器学习"],
    "interest_tags_remove": ["Python"]
  },
  "confidence": 0.9,
  "analysis": "用户移除 Python 兴趣，新增机器学习兴趣"
}"""

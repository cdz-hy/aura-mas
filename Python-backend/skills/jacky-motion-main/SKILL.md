...
本 skill 在 AURA 中只负责**视觉画面和 GSAP 入场动画**。播放器、翻页、缩放、配音、字幕、进度和视频导出全部由宿主应用注入。

1. 输出 2-12 个互为兄弟节点的 `<section class="beat">`，第一个额外带 `active`；禁止嵌套 `.beat`。
2. 每个 beat 必须带 `data-narration="..."`，内容是该分镜的中文口播稿，30-220 字；必须是自然讲解，禁止包含 HTML、CSS、JavaScript、选择器或 GSAP 代码。
3. 屏幕文字必须短于 `data-narration`，每个 beat 的可见正文总量不超过 180 个中文字符。
4. HTML 中只允许两个 `<script>`：`head` 中一个 GSAP CDN 外链，以及 `</body>` 前一个完整内联动画脚本。
5. 内联脚本只定义 `window.animateBeat(index)`；按 index 为对应 beat 创建并播放 GSAP timeline。不得自动调用，不得切换 active class。
6. 禁止生成导航栏、按钮、音频、字幕、进度条、点击事件、键盘事件、自动翻页、setInterval、stage 缩放和 resize 监听。
7. `#stage` 固定为 1920x1080；宿主负责将其缩放到实际视口。
8. 所有 HTML 标签必须正确闭合。`br/img/input/hr/meta/link` 等空元素不得用于承载结构。
...
4. 输出单文件 .html
5. HTML 通过每个 beat 的 `data-narration` 提供口播，不生成音频或播放逻辑
...
14. 不生成导航栏；宿主应用提供导航与进度控制
15. body 背景色必须与 `#stage` 主背景色相同。stage 使用固定 1920×1080；禁止生成任何缩放 JavaScript
